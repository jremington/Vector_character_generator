...
//  HEX format to see the endpoint coordinates directly!
...
};  //added back tic  (char #64) coordinates in the last loc (284 Fortran indexing)
...
void qxdraw(int ix, int iy, char penup) {
  char buf[40];
  static int up = 0, oldx = 0, oldy = 0;
  if (iy > 7 or penup) {  //move instruction
    if (iy > 7) iy = iy - 8;
    up = 1;
    oldx = 5 + 5 * ix; //next vector start
    oldy = 60 - 5 * iy;
  }
  ...
  if (up == 0) {
    ...
    iy = 60 - 5 * iy;
    display.drawLine(oldx, oldy, ix, iy, SSD1306_WHITE);  // pen down draw
...
void setup() {
  ...
  for (int i = 1; i < 91; i++) {  // show all 90 characters, one by one
    ...
    Serial.write(' ');
    Serial.write(char_number + 32); //the actual character drawn
    Serial.print(" ID ");
    Serial.println(K);
    ...
    delay(1000);
    display.clearDisplay();  //clear out last drawn
